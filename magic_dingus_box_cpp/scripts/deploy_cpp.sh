...
# Step 0.9: Normalize ownership of the deploy tree.
#
# rsync runs as `magic`, and it cannot set mtimes on files owned by root
# — it fails with "Operation not permitted" and exits 23, which under
# `set -euo pipefail` aborts the whole deploy before the build ever runs.
# Root-owned files land in the tree whenever something is installed or
# generated by a sudo/systemd process (magic-first-boot.service and
# import_library_movies.sh have both done it). Fixing ownership up front
# makes the deploy immune to that whole class of failure instead of
# playing whack-a-mole one filename at a time.
ssh "${PI_HOST}" "sudo chown -R magic:magic ${PI_DIR} 2>/dev/null || true"
