...
if [ "${MEDIA_BROWSER:-false}" = "true" ]; then
    ...
    # Exclude .env: the Pi's .env is authoritative (generated by
    # setup_services.sh); shipping a local dev .env would clobber the
    # Pi's real credentials and leak dev secrets.
