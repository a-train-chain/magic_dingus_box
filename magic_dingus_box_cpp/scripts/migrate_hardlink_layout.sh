...
CUR_ROOT="$(sqlite3 "${RADARR_DB}" "SELECT Path FROM RootFolders LIMIT 1;")"
...
LIB_COUNT_BEFORE="$(sqlite3 "${RADARR_DB}" "SELECT count(*) FROM Movies;")"
...
REMAIN="$(sqlite3 "${RADARR_DB}" "SELECT count(*) FROM Movies WHERE Path LIKE '/library/%';")"
INTEG="$(sqlite3 "${RADARR_DB}" "PRAGMA integrity_check;")"
LIB_COUNT_AFTER="$(sqlite3 "${RADARR_DB}" "SELECT count(*) FROM Movies;")"
[ "${REMAIN}" = "0" ]      || { cp -a "${BACKUP}" "${RADARR_DB}"; die "rewrite left ${REMAIN} /library paths — restored backup, aborted"; }
[ "${INTEG}" = "ok" ]      || { cp -a "${BACKUP}" "${RADARR_DB}"; die "integrity_check=${INTEG} — restored backup, aborted"; }
[ "${LIB_COUNT_AFTER}" = "${LIB_COUNT_BEFORE}" ] || { cp -a "${BACKUP}" "${RADARR_DB}"; die "movie count changed ${LIB_COUNT_BEFORE}->${LIB_COUNT_AFTER} — restored backup, aborted"; }
...
if [ -n "${RADARR_KEY}" ]; then
    ...
    log "If it dropped to 0/low, restore the backup: docker compose stop radarr && cp -a '${BACKUP}' '${RADARR_DB}' && docker compose up -d radarr"
else
