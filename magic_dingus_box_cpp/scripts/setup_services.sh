...
# posture. This degrades safely: with SONARR_KEY empty, the 9.6 readiness
...
#
# Mirrors the Sonarr probe in Step 9.6, and sits HERE for the same reason it
...
# 9.6. Sonarr API readiness probe — gates every hard-fail Sonarr step below.
#
# Twin of the Radarr probe in Step 9.5, and it sits HERE for the same reason:
# this is the last point before the script's first hard-fail Sonarr contact.
# (It used to live inside the Sonarr Custom Format block at Step 10-S; that
# block moved into converge_custom_formats.sh, which runs its own probes, so
# the SONARR_READY the later steps depend on is established here instead.)
#
# The probe result GATES Steps 14b-S / 15-S / 16-S / 17-S — it must not
# merely delay them. A bare for-loop-then-continue falls through identically
# whether Sonarr answered or never came up; dooming later SONARR steps on an
# unreachable Sonarr is fine and intended, dooming unrelated steps is not.
echo "Probing Sonarr API readiness..."
SONARR_READY=0
for i in {1..30}; do
    # --max-time 5 bounds the probe in WALL CLOCK, mirroring the two Radarr
    # probes. A container that is up with its port bound but the app wedged
    # mid-init accepts the socket and never answers — an untimed curl blocks
    # there forever and the "60s" loop becomes unbounded during first-boot
    # provisioning.
...
if [ "${SONARR_READY}" -ne 1 ]; then
    echo "  WARN: Sonarr not reachable after 60s — Sonarr-specific steps below will skip and self-heal on the next run."
else
    echo "  ✓ Sonarr API responding"
fi

# 10. Codify Radarr AND Sonarr Custom Formats + both "Any" profile score maps.
#
# The reconcilers themselves live in converge_custom_formats.sh, a
# standalone idempotent converge script, because they must ALSO run on every
# OTA. setup_services.sh runs at provisioning time and from the Content
# Manager's Media Browser Configure/Reconfigure flow — and nowhere else — so
# a release that adds or retunes a Custom Format used to ship the new
# fixture JSON to every fielded box (the tarball carries scripts/data/*.json)
# and then never reconcile it into Radarr/Sonarr. The box kept downloading
# against the OLD rules with the NEW rules sitting on disk beside them.
# update.sh now invokes the same script on every OTA; see
# OTA_UPDATE_GUARANTEES.md.
#
# CALLED, not copied: there is exactly one copy of each reconciler in the
# repo, so provisioning and OTA cannot drift apart. The script resolves its
# own API keys from ${SERVICES_DIR} and runs its own bounded readiness
# probes, and exits 0 for every "nothing to converge" condition (no .env, no
# key, service unreachable, fixture missing). A non-zero exit is a genuine
# failure and is deliberately left un-guarded here so it aborts provisioning
# exactly as the inline heredocs did under `set -euo pipefail`.
MAGIC_SERVICES_DIR="${SERVICES_DIR}" bash "${SCRIPT_DIR}/converge_custom_formats.sh"
