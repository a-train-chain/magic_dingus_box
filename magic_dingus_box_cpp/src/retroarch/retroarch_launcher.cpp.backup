...
namespace {
    ...
    ControllerMapping get_mapping_for_core(const std::string& core_name) {
        ...
        if (core_name.find("nestopia") != std::string::npos || core_name.find("fceumm") != std::string::npos) {
        ...
        } else if (core_name.find("mupen64") != std::string::npos || core_name.find("parallel_n64") != std::string::npos) {
            map.name = "N64 (Native)";
            map.analog_dpad_mode = "1"; // Enable analog stick for N64
            map.extra_config = "mupen64plus-next-pak1 = \"memory\"\n";
            
            // N64 A (ID 2) -> RetroPad A
            map.a_btn = "2";
            // N64 B (ID 1) -> RetroPad B
            map.b_btn = "1";
            // Start (ID 12) -> Start
            map.start_btn = "12";
            
            // Triggers
            map.l_btn = "4";  // L (ID 4) -> L1
            map.r_btn = "6";  // R (ID 6) -> R1
            map.l2_btn = "5"; // Z (ID 5) -> L2 (Z-Trigger)
            
            // C-Buttons (Mapped to Right Stick for compatibility)
            // OR mapped to buttons if using "Digital C-Buttons"
            // Let's map them to standard buttons first, which N64 cores usually interpret correctly
            // RetroPad Y -> C-Left
            map.y_btn = "0";  // C-Left (ID 0)
            // RetroPad X -> C-Up
            map.x_btn = "9";  // C-Up (ID 9)
            // RetroPad B (used for C-Down often?) No, standard mapping is usually right stick.
            
            // Let's try explicit C-Button mapping if supported, or standard Retropad layout
            // Retropad Layout:
            // B -> B
            // A -> A
            // Y -> C-Left
            ...
            // Actually, best way for N64 controllers on N64 cores:
            // Map C-Buttons to Right Analog Stick Axes? No, we have buttons.
            
            // Let's map them to the buttons we know:
            // C-Down (ID 3) -> RetroPad Select (often used for C-Down)
            map.select_btn = "3";
            
            // C-Right (Guess ID 8) -> RetroPad R2
            map.r2_btn = "8"; 
...
bool RetroArchLauncher::launch_game(const GameLaunchInfo& game_info) {
    if (!retroarch_available_) {
    ...
    return launch_drm(game_info);
}

bool RetroArchLauncher::launch_drm(const GameLaunchInfo& game_info) {
    ...
    std::cout << "Launching RetroArch via wrapper script" << std::endl;
    
    // Stop GStreamer and cleanup audio resources first
    stop_gstreamer_and_cleanup();

    // Build core name with _libretro suffix for wrapper script
    ...
    std::string service_name = service_name_env ? service_name_env : "magic-dingus-box-cpp.service";

    // Path to wrapper script
    std::string wrapper_script = "/opt/magic_dingus_box/scripts/launch_retroarch.sh";
    
    // Check if wrapper script exists
    if (!fs::exists(wrapper_script)) {
        std::cerr << "ERROR: Wrapper script not found: " << wrapper_script << std::endl;
        return false;
    }

    // Build command to call wrapper script
    // Arguments: ROM_PATH CORE OVERLAY_PATH SERVICE_NAME
    std::vector<std::string> cmd = {
        wrapper_script,
        game_info.rom_path,
        core_name,
        game_info.overlay_path.empty() ? "" : game_info.overlay_path,
        service_name
    };

    std::cout << "Calling wrapper script: " << wrapper_script << std::endl;
    std::cout << "  ROM: " << game_info.rom_path << std::endl;
    std::cout << "  Core: " << core_name << std::endl;
    std::cout << "  Overlay: " << (game_info.overlay_path.empty() ? "(none)" : game_info.overlay_path) << std::endl;
    std::cout << "  Service: " << service_name << std::endl;

    // Fork and exec to run the wrapper script
    pid_t pid = fork();
    ...
    if (pid == 0) {
        // Child process - exec the wrapper script
        ...
        execv(wrapper_script.c_str(), args.data());
        
        // If exec returns, it failed
        std::cerr << "ERROR: Failed to exec wrapper script: " << strerror(errno) << std::endl;
        _exit(1);
    }
    
    // Parent process - wait for wrapper script to complete
    std::cout << "Waiting for wrapper script (PID: " << pid << ") to complete..." << std::endl;
    
    int status;
    pid_t result = waitpid(pid, &status, 0);
    
    if (result < 0) {
        std::cerr << "ERROR: Failed to wait for wrapper script: " << strerror(errno) << std::endl;
        return false;
    }
    
    if (WIFEXITED(status)) {
        int exit_code = WEXITSTATUS(status);
        std::cout << "Wrapper script exited with code: " << exit_code << std::endl;
        
        if (exit_code == 0) {
            std::cout << "RetroArch launch successful" << std::endl;
            return true;
        } else {
            std::cerr << "Wrapper script failed with exit code: " << exit_code << std::endl;
            return false;
        }
    } else if (WIFSIGNALED(status)) {
        int signal = WTERMSIG(status);
        std::cerr << "Wrapper script terminated by signal: " << signal << std::endl;
        return false;
    }
    
    std::cerr << "Wrapper script exited abnormally" << std::endl;
    return false;
}


    std::string user_core_dir = "/home/alexanderchaney/.config/retroarch/cores";
    ...
    std::string launcher_script = "/tmp/retroarch_launcher.sh";
    {
        ...
        if (script_file.is_open()) {
            ...
            } else if (core_name.find("mupen64") != std::string::npos) {
                script_file << "mupen64plus-next-pak1 = \"memory\"\n";
                // Winner: Vulkan + ParaLLEl
                script_file << "mupen64plus-rdp-plugin = \"parallel\"\n";
                script_file << "mupen64plus-rsp-plugin = \"hle\"\n"; 
                script_file << "mupen64plus-NativeResFactor = \"1\"\n";
            }
            script_file << "OPTS\n";
            ...
            script_file << "libretro_system_directory = \"/home/alexanderchaney/.config/retroarch/system\"\n";
            
            // Dynamic Video Driver Selection
            if (core_name.find("mupen64") != std::string::npos || core_name.find("parallel_n64") != std::string::npos) {
                // N64 Fallback: GL + GLideN64 + No Threading + 640x480 (Test 3)
            script_file << "video_driver = \"gl\"\n";
                // Explicitly set KMS context
                script_file << "video_context_driver = \"kms\"\n"; 
                
                script_file << "video_threaded = \"false\"\n"; 
                script_file << "video_force_srgb_disable = \"true\"\n"; 
                script_file << "video_vsync = \"true\"\n"; 
                
                // CRITICAL: FORCE Core Options directly in main config (Redundancy)
                script_file << "mupen64plus-rdp-plugin = \"gliden64\"\n";
                script_file << "mupen64plus-rsp-plugin = \"hle\"\n"; 
                script_file << "mupen64plus-NativeResFactor = \"1\"\n";
                script_file << "mupen64plus-next-pak1 = \"memory\"\n";
                
                // Audio
                script_file << "audio_driver = \"alsathread\"\n"; 
                script_file << "audio_resampler = \"nearest\"\n"; 
            } else {
            ...
            // Resolution Logic
            if (core_name.find("mupen64") != std::string::npos || core_name.find("parallel_n64") != std::string::npos) {
                // N64: 320x240 not supported by KMS, using 640x480 (2x scaling)
...
//             script_file << "video_fullscreen_y = \"480\"\n";
            ...
            // CRITICAL: Overwrite persistent core options file which forces GLideN64
            // This file (Mupen64Plus-Next.opt) overrides our main config and temporary core options
            if (core_name.find("mupen64") != std::string::npos || core_name.find("parallel_n64") != std::string::npos) {
                script_file << "mkdir -p \"$HOME/.config/retroarch/config/Mupen64Plus-Next\"\n";
                script_file << "cat > \"$HOME/.config/retroarch/config/Mupen64Plus-Next/Mupen64Plus-Next.opt\" << 'OPT_EOF'\n";
                script_file << "mupen64plus-rdp-plugin = \"gliden64\"\n";
                script_file << "mupen64plus-rsp-plugin = \"hle\"\n";
                script_file << "mupen64plus-next-pak1 = \"memory\"\n";
                script_file << "mupen64plus-NativeResFactor = \"1\"\n";
                // Include other critical options to prevent overriding with defaults
                script_file << "mupen64plus-ThreadedRenderer = \"False\"\n"; 
                script_file << "OPT_EOF\n";
                script_file << "echo 'Launcher: Overwrote Mupen64Plus-Next.opt to force GLideN64' >> /tmp/retroarch_launcher.log\n";
            }
            ...
            script_file << "export HOME=/home/alexanderchaney\n";
            ...
            script_file << "exec 1>>/tmp/retroarch_launcher.log 2>&1\n";
        ...
        setenv("HOME", "/home/alexanderchaney", 1);
        setenv("DISPLAY", ":0", 1);
        ...
        if (launch_pid == 0) {
            ...
            int log_fd = open("/tmp/retroarch_launcher.log", O_WRONLY | O_CREAT | O_APPEND, 0644);
            if (log_fd != -1) {
...
bool RetroArchLauncher::open_core_downloader() {
    if (!retroarch_available_) {
        std::cerr << "RetroArch not available" << std::endl;
        return false;
    }
    ...
    return open_core_downloader_direct();
}

bool RetroArchLauncher::open_core_downloader_direct() {
