...
// to Detail.
//
// This file is the skeleton — sort/render/grab logic lands in Task 11.
...
public:
    ...
    // Set the candidates to display. Caller is responsible for raw data;
    // this screen handles sort + flag population in Task 11. Resets focus.
    void set_candidates(std::string movie_title,
                        std::vector<ReleaseCandidate> rows);

    // MbScreen overrides — implemented inert (no-op render, BACK→Detail
    // input) in this task; Task 11 fills them in.
    ...
    // Static helpers (separated for unit testing in Task 11).
