...
14. **Sonarr stopped bounds Movies mode to Sonarr's timeout, worst case ~5 s — this is expected, not a regression.** Radarr and Sonarr fetch concurrently in `run_library_refresh`, but there is exactly ONE publish point, after `sonarr_worker.join()` (see the comment there) — so Radarr's already-finished result waits behind Sonarr's. With `mdb_sonarr` stopped, enter Browse in Movies mode: posters, the IN LIBRARY hide, and DOWNLOADING badges should land within about 5 s (Sonarr's `get_library_checked()` timeout, no retry), not instantly. That is the correct bounded behavior, not "as promptly as with Sonarr running" — a clean per-service publish split was judged not worth the risk to the CAS single-flight guard and the For You deferred-hook ordering for this branch (final review, Fix 6). Restart Sonarr afterwards.
