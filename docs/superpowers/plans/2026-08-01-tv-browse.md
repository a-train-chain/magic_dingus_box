...
14. **Sonarr stopped costs no time in Movies mode.** With `mdb_sonarr` stopped, enter Browse in Movies mode and confirm posters, the IN LIBRARY hide and DOWNLOADING badges appear as promptly as they do with Sonarr running (the fetch is concurrent, so the 5 s Sonarr timeout must not delay them). Restart Sonarr afterwards.
