...
- `library_view.h` (pure layer first — this is where the tests live):
  ...
  - `std::vector<LibraryEntry> build_library_entries(const std::vector<Movie>&, const std::vector<Series>&, const std::unordered_set<int>& watched_movie_ids, const std::unordered_map<int,int>& tv_watched_counts, const std::unordered_set<MediaRef>& downloading_refs)` — movie entry per Movie (file_count = has_file?1:0, total 1, watched from set); TV file/total counts use the SAME season-0-excluded basis as WatchStore::tv_watched_counts: sum Season::episode_file_count / episode_count over series.seasons where season_number > 0 (already parsed; the series-level statistics.episodeFileCount INCLUDES imported S0 specials and would leave such a series permanently un-watchable — Task 3 reviewer adjudication). The series-level stat may still drive the inclusion rule only. Series entry ONLY when `episode_file_count > 0 || downloading_refs.count(tv ref)` (spec inclusion rule); sorted stays caller's job.
