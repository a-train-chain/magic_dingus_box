...
- **Next-episode resolution is pure** (`episode_logic.h`): `next_up(episodes, watch_rows) → optional<EpisodeInfo>` — first unwatched-with-file after the current episode in airing order, crossing season boundaries when files exist. The countdown needs the episode list: SeriesDetail passes the loaded episode vector through the dispatcher handoff (`PlayTarget` analog), so playback never fetches.
...
- Dual-board: no new decode paths (same playbin pipeline); runtime branching only; sqlite3 already links into the kiosk via `MEDIA_BROWSER_SOURCES`.
